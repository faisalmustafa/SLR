...
$$ p_o = \frac{(9 + 7)}{30} = \frac{16}{30}=0.533 $$
...
$$ \text{Expected to include} = \frac{12}{30} . \frac{20}{30} = 0.267 $$
...
$$ \kappa = \frac{p_o - p_e}{1 - p_e} = \frac{0.533 - 0.467}{1 - 0.467} = 0.12$$
...
Calculating kappa is straightforward using R. We will use *psych* package to perform the calculation. First, let's reproduce the data I used in this material.
...
Furthermore, install *psych package if you do not have it yet and run the calculation. We unly use the data in column 2 and 3, referring to Rater_1 and Rater_2 for this calculation because Cohen's kappa is used when only two raters are incolved. Please look for unweighted kappa for the result.

```{r cache = TRUE}
#install.packages("psych")
psych::cohen.kappa(kappa_data[,2:3])
```
